...
public class XadesEstStrategy extends XadesBase implements EstStrategy {
    private final SignInfo signInfo;

    public XadesEstStrategy(SignInfo signInfo) {
    ...
    public SignResult<Object> signEst() {
        ...
        if(tsServerUserId == null || tsServerUserId.equals(""))
        {
            ...
            TimestampConfig tsConfig = new TimestampConfig(tsSettings.getHostUrl(), String.valueOf(tsSettings.getCustomerID()), tsSettings.getCustomerPassword(), DigestAlg.SHA256);
        ...
        String signedFilePath = signInfo.getFile().getAbsolutePath() + ".EST.xml";
            try (FileOutputStream fileOutputStream = new FileOutputStream(signedFilePath)) {
                signature.write(fileOutputStream);
            }

        XadesValidationInfo validationInfo = XadesValidationInfo.builder().workingDirectory(workingDirectory).filePath(signedFilePath).build();
        baseValidate(validationInfo);

        signResult = SignResult.builder().documentPath(signInfo.getFile().getAbsolutePath() +".sign.xml")
            //.validationResult(validationResult)
            .build();
        } catch (IOException | LoginException | SmartCardException | XMLSignatureException | ConfigurationException ex) {
            System.out.println(ex);
        } 
        
        System.out.println("XADES EST imza atıldı.");
